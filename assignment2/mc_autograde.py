    ...
    for i in tqdm(range(num_episodes)):
        ...
        for t in range(len(states)-1, -1, -1):
            ...
            if states[t] not in states[:t]:
                ...
                V[states[t]] += (G - V[states[t]]) / returns_count[states[t]]
...
def mc_importance_sampling(behavior_policy, target_policy, env, num_episodes, discount_factor=1.0,
    ...
    for i in tqdm(range(num_episodes)):
        ...
        for t in range(len(states)-1, -1, -1):
            ...
            W *= target_probs[actions[t]] / behavior_probs[actions[t]]    
    return V
...
class SimpleBlackjackPolicy(object):
    ...
    def get_probs(self, states, actions):
        ...
        for s, a in zip(states, actions):
            if s[0] >= 20:
                probs[a] = 1 if a == 0 else 0
            else:
                probs[a] = 1 if a == 1 else 0
...
class SimpleBlackjackPolicy(object):
    ...
    def get_probs(self, states, actions):
        ...
        #         else:
        #             probs[a] = 0
        for idx, s, a in enurmerate(zip(states, actions)):
            if s[0] >= 20:
                probs[idx] = 1 if a == 0 else 0
            else:
                probs[idx] = 1 if a == 1 else 0
...
class SimpleBlackjackPolicy(object):
    ...
    def get_probs(self, states, actions):
        ...
        #         else:
        #             probs[a] = 0
        for idx, s in enumerate(states, actions):
            if s[0] >= 20:
                probs[idx] = 1 if actions[idx] == 0 else 0
            else:
                probs[idx] = 1 if actions[idx] == 1 else 0
...
def mc_importance_sampling(behavior_policy, target_policy, env, num_episodes, discount_factor=1.0,
    ...
    for i in tqdm(range(num_episodes)):
        ...
        for t in range(len(states)-1, -1, -1):
            ...
            behavior_probs = behavior_policy.get_probs(states, actions)
            # print(target_probs, behavior_probs, actions[t])
            W *= target_probs[t] / behavior_probs[t]    
    return V
...
def mc_importance_sampling(behavior_policy, target_policy, env, num_episodes, discount_factor=1.0,
    ...
    for i in tqdm(range(num_episodes)):
        ...
        for t in range(len(states)-1, -1, -1):
            ...
            # if actions[t] != target_policy.sample_action(states[t]):
            #     break
            target_probs = target_policy.get_probs(states[t], actions[t])
            behavior_probs = behavior_policy.get_probs(states[t], actions[t])
            # print(target_probs, behavior_probs, actions[t])
            W *= target_probs / behavior_probs  
            print(f"State: {states[t]} with action {actions[t]}, target: {target_probs}, behavior: {behavior_probs})
    return V
